...
ui <- fluidPage(
   ...
   # Application title
   titlePanel("将来予測と管理基準値計算(マサバ・ゴマサバ、太平洋・対馬)"),
   ...
   sidebarLayout(
       sidebarPanel(
         selectInput("species1",
                     "魚種・系群を選んでください",
                     list("系群" = c("マサバ(太平洋)","ゴマサバ(太平洋)",
                                     "マサバ(対馬)","ゴマサバ(東シナ海)")),
                     selected="ゴマサバ(東シナ海)"),
#         checkboxGroupInput("select2",
#                     "HCR",
#                     choices=list("default","なし"),
#                     selected="default"),
#         selectInput("select3",
#                     "新しい管理ルールによる漁獲",
#                     list("管理開始年"=as.character(c(0,1980:2010))),
#                     selected="0"),
         sliderInput("beta1",
                     "漁獲シナリオ(1)の安全係数β",
                     min=0,max=2.1,value=0.8),
         checkboxInput("is.HCR1",
                     "　　　(1)でBlimit以下でFを引き下げる",
                     value=FALSE),             
         sliderInput("beta2",
                     "漁獲シナリオ(2)の安全係数β",
                     min=0,max=2.1,value=1.3),
         checkboxInput("is.HCR2",
                     "　　　(2)でBlimit以下でFを引き下げる",
                     value=FALSE),                      
         textOutput("Fmsy_value"),
         checkboxInput("is.CI",
                     "信頼区間を表示する",
                     value=FALSE),
         checkboxInput("is.line",
                     "再生産関係の線を表示する",
                     value=FALSE),
         sliderInput("simu.number",
                     "加入のランダム変動の発生パターン（乱数）の違い",
                     min=1,max=100,value=50),
         hr(),
         h3("~説明~"),
         p("提案している漁獲管理規則のもとで、様々な安全係数βを用いたときの将来予測の結果（計算負荷の関係で、100回シミュレーションになるため、報告書の数字とは若干ずれます）。異なる２つの漁獲シナリオを比較できます。「Blimit以下で漁獲圧を引き下げる」のチェックを外すと、漁獲管理規則を使わない漁獲圧一定での将来予測になります。下は右の図の説明です。"),
         plotOutput("example",width="20%",inline=TRUE)                                    
      ),
      
      # Show a plot of the generated distribution
      mainPanel(
         plotOutput("out_select1",height="800px")
      )
   )
)
...
server <- function(input, output) {
    ...
    output$out_select1 <- renderPlot({    
        if(input$species1=="マサバ(太平洋)"){
            load("pms_files.rda")            
        }                        
        if(input$species1=="ゴマサバ(太平洋)"){
            load("pgs_files.rda")            
        }
        if(input$species1=="マサバ(対馬)"){
            load("tms_files.rda")
        }
        if(input$species1=="ゴマサバ(東シナ海)"){
            load("tgs_files.rda")            
        }

#        SR_res <- get(a[2])
#        MSY_res <- get(a[3])
        #        vpa_res <- MSY_res$input.list[[1]]$res0

        output$Fmsy_value <- renderText({         
            Fmsy <- round(1/as.numeric(as.character(MSY_res$summary$Fref2Fcurrent[1])),2)
            str_c("* 現状の漁獲の強さ (Fmsyの何倍か?):",Fmsy)
        })

        ## future projection
#        input.tmp <- future.default$input
        input.tmp$N <- 100 # ceiling(input$simu.number)+3
        ...
        if(!isTRUE(input$is.HCR1)) input.tmp1$HCR$Blim <-
            input.tmp1$HCR$Bban <- -Inf
        ...
        if(input$beta2>0){
            ...
            g3_future <- plot_futures(future1$input$res0,list(future1,future2),
                                      ...
                                      what.plot = c("Recruitment", "SSB", "catch","Fsakugen_ratio"),ncol=2)+
                ggtitle("Future projection")#+
            #                scale_color_manual(values = c("#f8766D","#00bfc4", "black"))
        }
        if(input$beta2==0){
            # ２つ(futureを1つ，VPAを1つ)をプロットする場合            
            g3_future <- plot_futures(future1$input$res0,
                                      list(future1),
                                      future.name=c(str_c("1. beta=",input$beta1)),
                                      biomass.unit=1000,n_example=1,
                                      is.plot.CIrange=input$is.CI,
                                      ...
                                      MSY=refs.plot$Catch[1],                                      
                                      what.plot = c("Recruitment", "SSB", "catch","Fsakugen_ratio"),ncol=2)+
                ggtitle(enc2utf8("Future projection"))+
                xlab("Year")#+
#                scale_color_manual(values = c("#f8766D","black"))                
        }       

        ## SRplot
        g1_SRplot <- SRplot_gg(SR_res,
                                xscale=1000,xlabel="1000 MT",
                                yscale=1,   ylabel="fish",
                                labeling.year=c(1990,2000,2010,2017),
                               add.info=FALSE) + theme_SH() +
            xlab("SB") + ylab("Recruit")+
            ggtitle("Stock-Recruitment")
            
        g1_SRplot <- g1_SRplot +
            geom_point(data=tibble(ssb=future1$vssb[,input$simu.number],
                                   recruit=future1$naa[1,,input$simu.number]),
                       mapping=aes(x=ssb/1000,y=recruit),color="#F8766D")
        if(isTRUE(input$is.line)){
            g1_SRplot <- g1_SRplot +
                geom_path(data=tibble(ssb=future1$vssb[,input$simu.number],
                                      recruit=future1$naa[1,,input$simu.number]),
                          mapping=aes(x=ssb/1000,y=recruit),color="#F8766D")
        }
        ...
        if(input$beta2>0){
            g1_SRplot <- g1_SRplot +
                geom_point(data=tibble(ssb=future2$vssb[,input$simu.number],
                                       recruit=future2$naa[1,,input$simu.number]),
                           mapping=aes(x=ssb/1000,y=recruit),color="#00BFC4")
            if(isTRUE(input$is.line)){
                g1_SRplot <- g1_SRplot +                
                geom_path(data=tibble(ssb=future2$vssb[,input$simu.number],
                                       recruit=future2$naa[1,,input$simu.number]),
                          mapping=aes(x=ssb/1000,y=recruit),color="#00BFC4")
            }            
        }
        ...
        ## yield curve
        g2_yield_curve <- plot_yield(MSY_res$trace,
                              refs.plot,
                              refs.label=c("Btarget","Blimit","Bban"),
                              future=list(future1,future2),
#                              past=vpa_res,
                              labeling=FALSE,
                              refs.color=c(1,1,1), # 印刷が出ないので縦線の色は黒にすることに
                              biomass.unit=1000,#資源量の単位
                              AR_select=FALSE,
                              future.replicate=rep(input$simu.number,2),
                              xlim.scale=0.6,ylim.scale=1.4 # x, y軸の最大値の何倍までプロットするか。ラベルやyield curveの形を見ながら適宜調整してください
                              ) + xlab("Average SB")+ ylab("Average Catch") + theme_SH() +
        ggtitle("Yield curve") +
        scale_color_manual(values = c("#f8766D","#00bfc4"))

        grid.arrange(g1_SRplot,g2_yield_curve,g3_future,
                     layout_matrix=t(matrix(c(1,2,3,3,3,3),2,3)))
        
    })

    output$example <- renderImage({
        list(src="example.png",alt=NULL,width="100%")        
    },deleteFile=FALSE)
...
## サーバーにアップ
#   library(rsconnect)
#   deployApp(lint=FALSE,appFiles=c("app.R","example.png","tms_files.rda","tgs_files.rda","pms_files.rda","pgs_files.rda"))


## if(0){
##     load("tms_files.rda")
##     res.tms <- input.tmp$res0
##     spr.tms <- get.SPR(res.tms)

##     load("pms_files.rda")
##     res.pms <- input.tmp$res0    
##     spr.pms <- get.SPR(res.pms)

##     pdf("SPR_comparison.pdf",height=4,width=4)
##     plot(colSums(res.pms$ssb),spr.pms$ysdata$SPR0,col=2,
##          xlab="SB",ylab="SPR",pch=2)
##     points(colSums(res.tms$ssb),spr.tms$ysdata$SPR0)
##     legend("bottomright",pch=1:2,col=1:2,legend=c("Tsushima","Pacific"))
